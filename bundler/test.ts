...
const senderCode = await sepoliaClient.getCode({
...
if(senderCode !== `0xef0100${delegateAddress.toLowerCase().substring(2)}`) {
    ...
    authorization = await sepoliaClient.signAuthorization(smartAccount.authorization)
}

const prepared = await sepoliaClient.prepareUserOperation({
...
const userOpHash = await sepoliaClient.sendUserOperation(prepared);
